    ...
    <div className="col-4" style={{ marginBottom: '20px' }}>
      <div className="card">
        <div className="card-body">
          ...
          <Link to={`/${(template.uniqueUrl)}`} className="card-text">{`http://localhost:3000/${(template.uniqueUrl)}`}</Link>
          <p className="card-text">{`Прогресс: ${progressCounter(template)} %`}</p>
          {/* <a type="button" onClick={clickHandler}
          className="btn btn-primary">Посмотреть лист</button> */}
